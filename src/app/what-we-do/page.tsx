          ...
          {programs.map((prog) => (
            <div key={prog.id} className="bg-white rounded-xl shadow-sm border border-brand-sage/50 overflow-hidden flex flex-col group hover:-translate-y-1 transition-all duration-300">
              <div className="h-48 bg-brand-sage relative flex items-center justify-center overflow-hidden">
                <Image 
                  src={`/images/programmes/card-${prog.id.toLowerCase()}.jpg`} 
                  alt={prog.title} 
                  fill 
                ...
                <span className={`absolute top-4 left-4 z-20 text-[9px] uppercase font-bold tracking-widest px-3 py-1.5 rounded border backdrop-blur-sm ${
                  prog.status === "Active" ? "bg-white/90 text-brand-green border-white" : 
